...
module.exports = class PlayCommand extends Command {
  ...
  async exec(msg, { query }) {
    try {
      ...
      if (!msg.member.voice.channelId) return msg.channel.send({ embeds: [CreateEmbed('warn', 'you must join voice channel to do this, peko.')] });
      if (!GuildPlayers) {
        ...
        if (MusicTracks.loadType === 'PLAYLIST_LOADED') {
          ...
          msg.channel.send({ embeds: [CreateEmbed('info', `Queued [${MusicTracks.tracks[0].title}](https://www.youtube.com/) [${msg.author}]`)] });
        }
        return player.play();
      }
      if (msg.member.voice.channelId !== GuildPlayers.voiceChannel) return msg.channel.send({ embeds: [CreateEmbed('warn', 'you must join voice channel same as me to do this, peko.')] });
      if (MusicTracks.loadType === 'PLAYLIST_LOADED') {
        for (const track of MusicTracks.tracks) {
      ...
      return msg.channel.send({ embeds: [CreateEmbed('info', `Queued [${MusicTracks.tracks[0].title}](https://www.youtube.com/) [${msg.author}]`)] });
    } catch (e) {
  ...
  async executeSlash(interaction) {
    ...
    if (!interaction.member.voice.channelId) return interaction.editReply({ embeds: [CreateEmbed('warn', 'you must join voice channel to do this, peko.')] });
    if (!GuildPlayers) {
      const player = await this.client.erela.create({
    ...
    if (interaction.member.voice.channelId !== GuildPlayers.voiceChannel) return interaction.editReply({ embeds: [CreateEmbed('warn', 'you must join voice channel same as me to do this, peko.')] });
    if (MusicTracks.loadType === 'PLAYLIST_LOADED') {
      for (const track of MusicTracks.tracks) {
        GuildPlayers.queue.add(track);
      }
